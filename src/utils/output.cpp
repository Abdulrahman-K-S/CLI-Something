 ...
 */
void typeWriter(string text, int delay_ms=1) {
    for (char c : text) {
        cout << c << flush;
        sleep_ms(delay_ms);
    }
...
/**
 * @brief typeWriter - Takes in some text and outputs them then delays the program a bit
 * 
 * @param text      Text to display
 * @param delay_ms  The delay inbetween each line and the next
 */
void lineWriter(string text, int delay_ms=1) {
    cout << text << flush;
    sleep_ms(delay_ms);
}
